...
cannonshell('theta')
print 'when velocity is 500m/s,','the optimum angle is 41.0, the max range of the cannon is',max(z),'m'
def can1(theta):
	v=500		
	vy=[]
	vx=[]
	...
	for i in range(500000):
		if y[-1]>=0:
			...
			break
	plt.plot(x,y,'b-',linewidth=1,label='the optimum angle is 41.0')
	plt.xlabel('x (m)')
	plt.ylabel('y (m)')
